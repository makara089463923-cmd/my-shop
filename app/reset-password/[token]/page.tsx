    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-pink-50 to-white flex items-center justify-center py-12 px-4">
        <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-6 sm:p-8 text-center">
          ...
          <Link href="/forgot-password" className="text-pink-500 hover:text-pink-600 text-sm font-medium">
            ស្នើសុំតំណភ្ជាប់ថ្មី
          </Link>
        </div>
      </div>
      ...
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-6 sm:p-8">
        ...
        {message ? (
          <div className="text-center">
            ...
            <Link
              href="/login"
              className="inline-block w-full bg-pink-500 text-white px-6 py-2 rounded-lg hover:bg-pink-600 transition text-center font-medium"
            >
              ចូលប្រើប្រាស់
            </Link>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                onChange={(e) => setPassword(e.target.value)}
                required
                className="w-full px-4 py-2 border border-gray-300 rounded-lg text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-pink-500 focus:border-transparent"
                placeholder="យ៉ាងហោចណាស់ 6 តួ"
              />
            </div>
            <div>
              ...
              <input
                type="password"
                value={confirmPassword}
                onChange={(e) => setConfirmPassword(e.target.value)} // ✅ បានកែសម្រួលឱ្យត្រឹមត្រូវហើយ
                required
                className="w-full px-4 py-2 border border-gray-300 rounded-lg text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-pink-500 focus:border-transparent"
                placeholder="បញ្ចូលពាក្យសម្ងាត់ម្តងទៀត"
              />
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-pink-500 text-white py-2 rounded-lg hover:bg-pink-600 transition disabled:opacity-50 font-medium"
            >
              {loading ? 'កំពុងរក្សាទុក...' : 'កំណត់ពាក្យសម្ងាត់'}
            </button>
          </form>
